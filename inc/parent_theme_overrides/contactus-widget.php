...
class pulsair_contact_widgets extends WP_Widget {
	function __construct() {
		$widget_ops = array( 'classname' => 'widget_contact', 'description' => __( 'Pulsair: Display Contact Us Information', 'freesia-empire') );
		$control_ops = array('width' => 200, 'height' => 250);
		parent::__construct( false, $name=__('Pulsair: Contact Us','freesia-empire'), $widget_ops, $control_ops );
	}
	...
	function widget( $args, $instance ) {
		...
		if(!empty($address1) || !empty($phone_no1) || !empty($phone_no2) || !empty($email_1) || !empty($skype_id1)): ?>
			<ul>
				...
				<?php endif;
				if(!empty($email_1)): ?>
					<li class="email"><a href="mailto:<?php echo antispambot(is_email($email_1)); ?>" title="<?php antispambot(is_email($email_1)); ?>"><?php echo antispambot(is_email($email_1)); ?></a></li>
				<?php endif;
				if(!empty($skype_id1)): ?>
